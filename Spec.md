...
1. [ ] `obj.method()` で引数なしメソッドが呼び出せる
2. [ ] `obj.method(arg1, arg2)` で引数ありメソッドが呼び出せる
3. [ ] メソッドの戻り値が正しく返される
4. [ ] メソッド内で `self.field` でフィールドを読み取れる
5. [ ] メソッド内で `self.field = value` でフィールドを更新でき、呼び出し元に反映される
6. [ ] メソッドチェーン `obj.method1().method2()` が動作する（method1が構造体を返す場合）
7. [ ] 既存のスナップショットテストがすべてパスする
8. [ ] 新規追加したメソッド呼び出しのスナップショットテストがパスする
