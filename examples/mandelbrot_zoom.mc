// Mandelbrot set - Zoomed into "Seahorse Valley"
// High iteration count reveals intricate fractal detail
// Resolution: 120x40, 500 iterations, 66-level gradient
...
fun print_char(idx: int) {
    ...
    if idx == 50 { print_str("d"); return; }
    if idx == 51 { print_str("b"); return; }
    if idx == 52 { print_str("k"); return; }
    if idx == 53 { print_str("h"); return; }
    if idx == 54 { print_str("a"); return; }
    if idx == 55 { print_str("o"); return; }
    if idx == 56 { print_str("*"); return; }
    if idx == 57 { print_str("#"); return; }
    if idx == 58 { print_str("M"); return; }
    if idx == 59 { print_str("W"); return; }
    if idx == 60 { print_str("&"); return; }
    if idx == 61 { print_str("8"); return; }
    if idx == 62 { print_str("%"); return; }
    if idx == 63 { print_str("B"); return; }
    if idx == 64 { print_str("@"); return; }
    print_str("$");
}
