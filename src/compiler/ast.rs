...
pub enum Statement {
    ...
    /// Match dyn statement: `match dyn expr { v: int => { ... } _ => { ... } }`
    MatchDyn {
        expr: Expr,
...
/// An arm in a match dyn statement.
#[derive(Debug, Clone)]
...
pub enum Expr {
    ...
    /// As dyn expression: `expr as dyn` — boxes a value with runtime type information.
    AsDyn {
        expr: Box<Expr>,
        span: Span,
        inferred_type: Option<Type>,
