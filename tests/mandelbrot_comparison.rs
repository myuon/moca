...
/// Run the moca mandelbrot implementation with a specific JIT threshold.
fn run_moca_mandelbrot_with_threshold(max_iter: i32, jit_threshold: u32) -> String {
    let moca_code = generate_moca_code(max_iter);

    // Write to a temporary file with unique name
    let temp_dir = std::env::temp_dir();
    let temp_file = temp_dir.join(format!(
        "mandelbrot_test_{}_{}_{}.mc",
        max_iter,
        jit_threshold,
        std::process::id()
    ));
    ...
    // Run the moca file with specified JIT threshold
...
/// Verify that JIT produces the same output regardless of threshold.
/// This catches bugs where JIT-compiled code produces different results than interpreter.
#[test]
fn mandelbrot_jit_threshold_consistency() {
    let max_iter = 100;
    let thresholds: [u32; 7] = [1, 2, 5, 10, 50, 100, 1000];

    // Get the baseline output (with high threshold, mostly interpreter)
    let baseline = run_moca_mandelbrot_with_threshold(max_iter, 1000);

    println!("\n=== JIT Threshold Consistency Test ===");
    println!("Baseline (threshold=1000): {} bytes", baseline.len());

    for threshold in &thresholds[..thresholds.len() - 1] {
        let output = run_moca_mandelbrot_with_threshold(max_iter, *threshold);
        assert_eq!(
            output, baseline,
            "Output differs with JIT threshold {}!\n\n--- threshold={} ---\n{}\n\n--- baseline (threshold=1000) ---\n{}",
            threshold, threshold, output, baseline
        );
        println!("✓ threshold={}: matches baseline", threshold);
    }

    println!("✓ All JIT thresholds produce consistent output!");
}
